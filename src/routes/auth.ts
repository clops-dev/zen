...
auth.post("/signup", async (c) => {
  ...
  const [newUser] = await withDbResilience(() => sql`
...
auth.post("/api-keys/:id/revoke", requireSession(), async (c) => {
  const session = c.var.session
  await withDbResilience(() => sql`UPDATE api_keys SET revoked = true WHERE id = ${c.req.param("id")} AND user_id = ${session.userId}`)
